...
{ 
    ...
    <input class="span1"style="max-width:34px"placeholder="'.$value.'" id="quantity'.$key.'"size="16" type="text">
          ...
          <td><span id="span1" class="'.$key.'1">'.$select["prix"].'</span>&euro;</td>
          <td><span id="span2" class="'.$key.'2">'.$select["prix"]*$value.'</span>&euro;</td>
          </tr>
          ');
      }
}
...
<script type="text/javascript">
  ...
  $('div[class="input-append"]').each(function(){
    ...
    $(btnplus).click(function(event){
      ...
      /*
      //debut
      var items=$('span[id="items"]').text();
      items=parseInt(items);
      items++;
      $('span[id="items"]').text(items);
      //fin
      */
      //changer panier
